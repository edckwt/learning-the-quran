...
class app_ltq_models {
	...
	{
		global $opicltq_categories_lang;
		$return = array();
		$table = $this->wpdb->prefix.'options';
		$like = OPICLTQ_Input_SLUG."category_".get_option(OPICLTQ_Input_SLUG.'language').'_';
		...
		if(!empty($data) && is_array($data)){
			foreach ($data as $key => $value) {
				...
				$return[$key]['option_url'] = $opicltq_categories_lang[get_option(OPICLTQ_Input_SLUG.'language')][$return[$key]['option_slug']]['importurl'];
